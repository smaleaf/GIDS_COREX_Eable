...
| API | 功能 |
|-----|------|
| `cuFileDriverOpen` | 初始化 cuFile 驱动（依赖 `itrfs.ko` 内核模块 + `/dev/itrfs` 设备节点） |
...
> **GDS 加速需要 `itrfs.ko`（Iluvatar corex GDS driver，等价于 NVIDIA `nvidia-fs.ko`）已加载且 `/dev/itrfs` 设备节点可访问。** 无此驱动时自动降级为 POSIX read。
...
# 加载 cuFile GDS 内核驱动（如果没有自动加载）
modprobe iluvatar                              # 主 GPU 驱动
insmod /path/to/ixdriver/kmd/itr_fs/itrfs.ko   # GDS 驱动
# 确认设备节点（若无则手动创建）
ls -la /dev/itrfs || mknod /dev/itrfs c $(grep itrfs /proc/devices | awk '{print $1}') 0
