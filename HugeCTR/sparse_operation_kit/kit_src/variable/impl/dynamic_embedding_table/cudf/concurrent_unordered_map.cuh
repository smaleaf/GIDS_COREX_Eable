...
class concurrent_unordered_map : public managed {
 ...
 public:
  ...
  __forceinline__ __device__ iterator insert_or_assign(const value_type &x, aggregation_type op,
  ...
  template <typename aggregation_type, typename counter_type, class comparison_type = key_equal,
            typename hash_value_type = typename Hasher::result_type>
  __forceinline__ __device__ iterator lookup_or_insert(const key_type &k, aggregation_type op,
