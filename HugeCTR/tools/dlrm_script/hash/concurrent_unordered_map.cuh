...
// CUDF_GE_2306: from cudf 23.06 moves the new file and the `hash_functions.cuh`
 ...
 **/
template <typename pair_type, typename key_type = typename pair_type::first_type,
...
class concurrent_unordered_map {
 ...
 public:
  ...
  void assign_async(const concurrent_unordered_map& other, cudaStream_t stream = 0) {
    ...
    CUDA_TRY(cudaMemcpyAsync(m_hashtbl_values, other.m_hashtbl_values,
                             m_capacity * sizeof(value_type), cudaMemcpyDefault, stream));
  }
 ...
 private:
  ...
  concurrent_unordered_map(size_type capacity, const mapped_type unused_element,
                           const key_type unused_key, const Hasher& hash_function,
                           const Equality& equal, const allocator_type& allocator,
